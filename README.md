...
面向 Unreal 项目 Static Mesh 的只读批量审计 MVP。项目 Profile 定义预算和预期，Editor-only C++ 模块批量采集元数据，Python 负责规则编排与 JSON 报告。扫描接口不保存资产、不重建网格，也不修改 Nanite。
...
- UE 原生中文 Slate 面板：读取 Content Browser 选择、从预置规则下拉框切换 Profile、运行审计、搜索问题与打开报告；
...
- 后续扩展应使用更接近项目内容的数据集复测，并保持不自动修改 Nanite 的当前边界。

持续开发状态见 [`config/goal-state.json`](config/goal-state.json)，完整路线见
