...
- 只接受当前模型 Profile v3、纹理 Profile v1 与 Material Interface Profile v1；
...
材质工作台使用独立的 Material Interface Profile v1 字段，包括 Material Domain、Blend Mode、双面、
实例父级、父级深度、纹理依赖数量与最大尺寸；它不会把材质字段塞进模型合同。
...
独立 UE 5.8.1 宿主验证了模型、纹理和材质标准的合同解析与字段编辑；模型和纹理标准还完成了
保存后真实采集，材质轨道完成了 9 个真实 Material Interface 的只读采集。自动化没有模拟人工鼠标录制，
