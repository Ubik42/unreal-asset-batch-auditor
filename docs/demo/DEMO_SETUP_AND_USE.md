...
5. 若要演示项目规则治理，点击“复制为项目标准”并打开“项目验收标准工作台”：修改版本或阈值，先预览右侧差异，再保存；非法字段会就地显示中文原因并保持保存禁用；
6. 将单批资产数设为 8，点击“开始只读审计”；
7. 观察左下任务卡：阶段、对象进度、批次进度会随 Editor Tick 更新；面板不会在扫描期间伪装成已完成；
8. 审计完成后默认进入“交付热区”。先看 6 个 Report 目录组：两个采集失败组、Developers、Heavy、Medium 和 Light；
9. 选择 `03_Heavy`，点击“查看组内问题”，确认下钻条说明只筛选当前 Report、不会重新扫描 Content Browser；
10. 点击“清除下钻”，再进入“资产总览”，确认每个资产的状态、LOD0 三角形、顶点、材质/纹理、LOD、Nanite、碰撞、LM UV、LM 分辨率与问题数；
11. 切换“问题明细”，用搜索框筛选规则、证据说明或资产名，并对照实测值与 Profile 阈值；
12. 点击“打开最新报告”直接查看 JSON，或点击“打开报告目录”进入本次报告所在目录。
