...
当前 `prepare_demo.ps1` 已切换到三套 v3 Profile，并在上述十类规则之外加入缺失材质、唯一材质、
纹理依赖和最大纹理尺寸。由于这些事实来自本机 Engine 资产，录制时应以新生成 Report 的实际统计为准，
不复用上表的 v2 固定 Issue 数；v2 报告仅作为合同兼容与历史回归证据保留。
