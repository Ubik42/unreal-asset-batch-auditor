...
然后运行 scripts/goal.ps1 -Action Doctor，只执行 nextSlice 中明确允许的范围。每轮只推进一个可验证切片；先修复已有失败，再开发新能力。切片结束必须运行 quick validation；需要 UE 的切片还必须完成 BuildPlugin，并用独立、隐藏、带超时的短生命周期 Unreal 进程验证。启动前记录已有 Unreal PID，只清理本轮创建且已确认归属的 PID；不得接管、切换、关闭或复用用户已有 UE 会话。

循环恢复规则：
- goal-state 是当前状态唯一来源，checkpoint 是不可改写的历史；两者冲突时停止并修复状态，不凭聊天记忆猜测；
- 任何失败都记录实际命令、退出状态、证据上限和最小恢复条件；失败三轮且无法继续时才标 blocked；
- 只把实际通过的验收项写入 checkpoint，不用计划文本代替完成证据；
- 工作区有非本切片改动时保留并绕开，不回滚、不覆盖；
- 每个切片完成后先更新文档与证据，再将 nextSlice 推进到唯一后继切片，最后提交并推送。
...
- 使用自合成或许可清晰的公开测试素材，提供足量真实当前版本截图、安装教程、录屏脚本和已知限制；
